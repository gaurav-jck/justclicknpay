...
class AEPSTxnDetailFragment : Fragment() {
    private var dmtResponse: AepsDetailReceiptResponse.data? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        arguments?.let {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                dmtResponse = it.getSerializable(ARG_PARAM1, AepsDetailReceiptResponse.data::class.java)
            }else{
                dmtResponse = it.getSerializable(ARG_PARAM1) as AepsDetailReceiptResponse.data
            }
        }
    }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        binding!!.title.setText("AEPS Receipt")
        if(dmtResponse!=null){
//            Toast.makeText(context, "Success", Toast.LENGTH_LONG).show()
    ...
    private fun setDmtReceiptData(view: View, dmtResponse: AepsDetailReceiptResponse.data) {
        binding!!.agentCodeTv.text = dmtResponse.doneCardUser
        binding!!.bankNameTv.text = dmtResponse.bankName
//        binding!!.accountNoTv.text = dmtResponse.accountNumber
        binding!!.mobileNoTv.text = dmtResponse.customerMobile
        binding!!.apiTxnIdTv.text = dmtResponse.apiRefrenceId
        binding!!.jckTxnIdTv.text = dmtResponse.reservationId
        binding!!.bankRefNoTv.text = dmtResponse.rrn
        binding!!.remitAmountTv.text = dmtResponse.txnAmount
        binding!!.availBalTv.text = ""
        binding!!.txnTypeTv.text = dmtResponse.paymentType
        binding!!.txnStatusTv.text = dmtResponse.apiMessage
...
//                            Toast.makeText(context,response.statusMessage, Toast.LENGTH_LONG).show()
    ...
    companion object {
        ...
        @JvmStatic
        fun newInstance(data: AepsDetailReceiptResponse.data) =
                AEPSTxnDetailFragment().apply {
