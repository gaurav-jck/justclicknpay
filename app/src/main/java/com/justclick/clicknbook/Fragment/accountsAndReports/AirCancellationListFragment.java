...
import com.justclick.clicknbook.adapter.AirCancellationListAdapter;
import com.justclick.clicknbook.model.AirCancellationListModel;
import com.justclick.clicknbook.model.LoginModel;
import com.justclick.clicknbook.myinterface.ToolBarTitleChangeListener;
import com.justclick.clicknbook.requestmodels.AirCancellationListRequestModel;
...
public class AirCancellationListFragment extends Fragment implements View.OnClickListener{
    ...
    private ArrayList<AirCancellationListModel.AirCancelListData> airCancelListDataArrayList;
    private AirCancellationListAdapter airCancellationListAdapter;
    ...
    private AirCancellationListRequestModel cancelListRequestModel;
    ...
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        context=getActivity();
        airCancelListDataArrayList =new ArrayList<>();
        cancelListRequestModel = new AirCancellationListRequestModel();
    ...
    public View onCreateView(LayoutInflater inflater, final ViewGroup container,
        ...
        titleChangeListener.onToolBarTitleChange(getString(R.string.airCancelListFragmentTitle));

        //initialize date values
        setDates();

        airCancellationListAdapter =new AirCancellationListAdapter(context, new AirCancellationListAdapter.OnRecyclerItemClickListener() {
            @Override
            public void onRecyclerItemClick(View view, ArrayList<AirCancellationListModel.AirCancelListData> list, int position) {

            }
        }, airCancelListDataArrayList, totalPageCount);
        layoutManager=new LinearLayoutManager(context);
        recyclerView.setLayoutManager(layoutManager);
        recyclerView.setAdapter(airCancellationListAdapter);

        if(airCancelListDataArrayList !=null && airCancelListDataArrayList.size()==0) {
            pageStart=1;
            pageEnd=10;
...
//            }
    ...
    public void serviceCall(AirCancellationListRequestModel reportListRequestModel, boolean progress) {
        if(progress){
            showCustomDialog();
        }
        ApiInterface apiService =
                APIClient.getClient().create(ApiInterface.class);
        Call<AirCancellationListModel> call = apiService.getAirCancelListPost(ApiConstants.AirCancellationReport, reportListRequestModel);
        call.enqueue(new Callback<AirCancellationListModel>() {
            @Override
            public void onResponse(Call<AirCancellationListModel>call, Response<AirCancellationListModel> response) {
//                Toast.makeText(context, "response ", Toast.LENGTH_LONG).show();
                try{
                    hideCustomDialog();
                    if(response!=null){
                        if(response.body().StatusCode.equalsIgnoreCase("0")){
                            airCancelListDataArrayList.addAll(response.body().Data);
                            airCancellationListAdapter.notifyDataSetChanged();
...
//                            airCancellationListAdapter.notifyDataSetChanged();
                            ...
                            airCancelListDataArrayList.clear();
                            airCancellationListAdapter.notifyDataSetChanged();
                            Toast.makeText(context,response.body().Status, Toast.LENGTH_LONG).show();
                        }
                    }else {
...
//                    Toast.makeText(context, R.string.exception_message, Toast.LENGTH_LONG).show();
            ...
            @Override
            public void onFailure(Call<AirCancellationListModel> call, Throwable t) {
                hideCustomDialog();
                Toast.makeText(context, R.string.response_failure_message, Toast.LENGTH_LONG).show();
            }
