...
import com.justclick.clicknbook.databinding.FragmentRechargeDetailReceiptBinding
...
class RechargeTxnDetailFragment : Fragment() {
    private var dmtResponse: RechargeDetailReceiptResponse.data? = null
    private var param2: String? = null
    private var loginModel: LoginModel?=null
    var binding: FragmentRechargeDetailReceiptBinding?=null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        arguments?.let {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                dmtResponse = it.getSerializable(ARG_PARAM1, RechargeDetailReceiptResponse.data::class.java)
            }else{
                dmtResponse = it.getSerializable(ARG_PARAM1) as RechargeDetailReceiptResponse.data
            }
        }
    }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        var view= inflater.inflate(R.layout.fragment_recharge_detail_receipt, container, false)
        binding= FragmentRechargeDetailReceiptBinding.bind(view)
        if(dmtResponse!=null){
//            Toast.makeText(context, "Success", Toast.LENGTH_LONG).show()
    ...
    private fun setDmtReceiptData(view: View, dmtResponse: RechargeDetailReceiptResponse.data) {
        binding!!.agentCodeTv.text = dmtResponse.agentCode
        binding!!.agencyTv.text = dmtResponse.agencyName
        binding!!.custNameTv.text = dmtResponse.cName
        binding!!.accountNoTv.text = dmtResponse.rechargeNumber
        binding!!.remitAmountTv.text = dmtResponse.amount
        binding!!.apiTxnIdTv.text = dmtResponse.apiRefNumber
        binding!!.jckTxnIdTv.text = dmtResponse.transactionId
        binding!!.remitAmountTv.text = dmtResponse.amount
        binding!!.txnTypeTv.text = dmtResponse.category+" "+dmtResponse.rechargeType
...
//                            Toast.makeText(context,response.statusMessage, Toast.LENGTH_LONG).show()
    ...
    companion object {
        ...
        @JvmStatic
        fun newInstance(data: RechargeDetailReceiptResponse.data) =
                RechargeTxnDetailFragment().apply {
