...
import com.justclick.clicknbook.adapter.AirSalesReportAdapter;
...
public class AirSalesReportFragment extends Fragment implements View.OnClickListener{
    ...
    private ArrayList<AirSalesReportModel.Data> airSalesReportDataArrayList;
    private AirSalesReportAdapter airSalesReportAdapter;
    ...
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        context=getActivity();
        airSalesReportDataArrayList =new ArrayList<>();
    ...
    public View onCreateView(LayoutInflater inflater, final ViewGroup container,
        ...
        titleChangeListener.onToolBarTitleChange(getString(R.string.airSalesReportFragmentTitle));

        //initialize date values
        setDates();

        airSalesReportAdapter = new AirSalesReportAdapter(context, new AirSalesReportAdapter.OnRecyclerItemClickListener() {
            @Override
            public void onRecyclerItemClick(View view, ArrayList<AirSalesReportModel.Data> list, int position) {

            }
        }, airSalesReportDataArrayList, totalPageCount);
        layoutManager=new LinearLayoutManager(context);
        recyclerView.setLayoutManager(layoutManager);
        recyclerView.setAdapter(airSalesReportAdapter);

        if(airSalesReportDataArrayList !=null && airSalesReportDataArrayList.size()==0) {
            pageStart=1;
            pageEnd=10;
...
//            }
    ...
    public void serviceCall(AirRefundReportRequestModel reportListRequestModel, boolean progress) {
        ...
        Call<AirSalesReportModel> call = apiService.getAirSalesReportPost(ApiConstants.AirSalesReport, reportListRequestModel);
        call.enqueue(new Callback<AirSalesReportModel>() {
            @Override
            public void onResponse(Call<AirSalesReportModel>call, Response<AirSalesReportModel> response) {
//                Toast.makeText(context, "response ", Toast.LENGTH_LONG).show();
                try{
                    hideCustomDialog();
                    if(response!=null){
                        if(response.body().StatusCode.equalsIgnoreCase("0")){
                            airSalesReportDataArrayList.addAll(response.body().Data);
                            airSalesReportAdapter.notifyDataSetChanged();
...
//                            airSalesReportDataArrayList.clear();
//                            airSalesReportAdapter.notifyDataSetChanged();
                            Toast.makeText(context,response.body().Status, Toast.LENGTH_LONG).show();
                        }
                    }else {
...
//                    Toast. makeText(context, R.string.exception_message, Toast.LENGTH_LONG).show();
            ...
            @Override
            public void onFailure(Call<AirSalesReportModel> call, Throwable t) {
                hideCustomDialog();
                Toast.makeText(context, R.string.response_failure_message, Toast.LENGTH_LONG).show();
            }
